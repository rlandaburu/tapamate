gsap.registerPlugin(ScrollTrigger);
    ...
    //animate text
    scene1.to("#info", { y: 10 * speed , x: 3* speed}, 0)
...
const fadeImage = document.getElementById('fade-in-image');

// Check if the bottom of the image is within 200px of the bottom of the viewport
function checkPosition() {
  const imageBottom = fadeImage.getBoundingClientRect().bottom;
  const viewportBottom = window.innerHeight;
  if (imageBottom < viewportBottom - 200) {
    fadeImage.style.opacity = 1;
  }
}

// Call the checkPosition function whenever the window is scrolled
